...
|-------|----------------|--------------|
| `/complete-updatelog` | **Completes Updatelog** | This skill only runs if you explicitly ask it to, (/complete-updatelog). Works through the numbered Stages of one or more UPDATELOGV<M>.md docs — one stage at a time, never combined. Does the work, runs the stage's Verify: steps, fills in the Stage N Report, then commits + pushes as stage<N>v<M> after each stage. Runs on an `updatelogv<M>` branch and **asks at the end whether to merge into main**, writing the merge description itself in your existing voice. **Respects a daily contribution-graph budget** (`DAILY_LIMIT=47`, merges counted) — at the ceiling it finishes the current stage, commits it, and defers the rest to a jittered 1–6am slot instead of blowing past. Always closes with a copy-paste **NEXT UP** block for a fresh context. Also `next` (auto-pick the next unfinished log), `--status` (read-only stage table), `--flush` (resume a deferred run), `--no-limit`, `--no-merge`. |
| `/complete-updatelog-nocommit` | **Completes Updatelog (no commit)** | Same as `/complete-updatelog`, minus all git writes. Only runs if you explicitly ask it to (/complete-updatelog-nocommit). Works the Stages one at a time, verifies, fills in each Stage N Report — and never commits, pushes, stashes or resets. Everything is left uncommitted in the working tree; the summary lists the changed files so you can commit yourself, and the **NEXT UP** block names the commit messages the committing version would have used. Also takes `next` and `--status`. No branch, no merge, no budget — there are no commits to pace. |
...
| `/founder-brief` | **The Briefer** | Generates an interrogation-ready founder's briefing for the current project — the 2-3 page doc you'd hand someone who walks up and asks "I want to invest," "what does it do?", or "what are your next stages?" Reads the repo, runs the build/tests for real evidence, tags every feature ✅ verified / ⚠️ done-untested / ○ stubbed, and answers each question a person would ask, cold. Works on any project from greenfield to live. Writes founder-brief.md, then offers a shareable Artifact link and a PDF. Regenerated fresh every run, never stored as truth — read-only on your product code. |
